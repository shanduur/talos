...
diskSelector:
    ...
    # busPath: /pci0000:00/*
{{< /highlight >}}</details> | |
|`mirror` |bool |Install Talos to a software RAID1 mirror across every disk matched by<br>`diskSelector` (which must match at least two disks) instead of a<br>single disk. Requires the `mdadm` system extension. Mutually exclusive<br>with `disk`.  |`true`<br />`yes`<br />`false`<br />`no`<br /> |
