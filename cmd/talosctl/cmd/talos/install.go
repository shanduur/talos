...
func installInternal(ctx context.Context, c *client.Client, disk string) error {
	...
	stream, err := c.LifecycleClient.Install(ctx, &machine.LifecycleServiceInstallRequest{
		...
		Options: &machine.InstallOptions{
			Wipe:            installCmdFlags.wipe,
			ExtraKernelArgs: installCmdFlags.extraKernelArgs,
		},
	})
	if err != nil {
...
func init() {
	...
	installCmd.Flags().BoolVar(&installCmdFlags.wipe, "wipe", false, "whether to wipe the disk before installation")
	installCmd.Flags().StringArrayVar(&installCmdFlags.extraKernelArgs, "extra-kernel-args", nil, "extra kernel arguments to add to the installed system")
