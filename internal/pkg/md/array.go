...
//
// Reading sysfs needs no mdadm binary, so this works regardless of the
// extension; force-running the result does need mdadm (see RunArray).
...
func parseDetailExport(out string) Detail {
	...
	for line := range strings.SplitSeq(out, "\n") {
		...
		switch {
		case key == "MD_DEVICES":
			d.RaidDevices, _ = strconv.Atoi(value)
