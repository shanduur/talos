...
//
// Healthy arrays are assembled automatically by the mdadm extension's udev
// rules; this controller only handles the degraded case, which on systemd is
// covered by mdadm-last-resort@.timer. Without it, a RAID1 whose mirror disk
// failed would never start and the node could not boot from the survivor.
//
// mdadm ships in a system extension, not core Talos; if the binary is absent
// the controller is a no-op (a node that cannot create arrays has none to run).
...
// mdInstance lazily resolves the mdadm wrapper, caching the ErrNotInstalled
// verdict so it is only paid once.
...
func (ctrl *MDLastResortController) Run(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	for {
		...
		case <-graceCh:
			...
			if err != nil {
				if errors.Is(err, md.ErrNotInstalled) {
					return nil
				}
